...
CataCamera::Parameters::writeToYamlFile( const std::string& filename ) const
{
    ...
    fs << "{"
       << "gamma1" << m_gamma1 << "gamma2" << m_gamma2 << "u0" << m_u0 << "v0" << m_v0 << "}";
...
void
CataCamera::liftProjective( const Eigen::Vector2d& p, Eigen::Vector3d& P, float image_scale ) const
{
...
void
CataCamera::spaceToPlane( const Eigen::Vector3d& P, Eigen::Vector2d& p, float image_scalse ) const
{
 ...
 */
void
CataCamera::spaceToPlane( const Eigen::Vector3d& P, Eigen::Vector2d& p, Eigen::Matrix< double, 2, 3 >& J ) const
{
    double xi = mParameters.xi( );
    ...
    // Apply generalised projection matrix
    p << gamma1 * p_d( 0 ) + mParameters.u0( ), gamma2 * p_d( 1 ) + mParameters.v0( );
 ...
 */
void
CataCamera::distortion( const Eigen::Vector2d& p_u, Eigen::Vector2d& d_u, Eigen::Matrix2d& J ) const
{
    double k1 = mParameters.k1( );
    double k2 = mParameters.k2( );
    double p1 = mParameters.p1( );
...
cv::Mat
CataCamera::initUndistortRectifyMap(
