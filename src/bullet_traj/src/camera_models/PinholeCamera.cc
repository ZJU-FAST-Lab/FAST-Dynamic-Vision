...
PinholeCamera::Parameters::Parameters(
const std::string& cameraName, int w, int h, double k1, double k2, double p1, double p2, double fx, double fy, double cx, double cy )
...
void PinholeCamera::liftProjective( const Eigen::Vector2d& p, Eigen::Vector3d& P, float image_scale ) const
{
...
void
PinholeCamera::spaceToPlane( const Eigen::Vector3d& P, Eigen::Vector2d& p, float image_scalse ) const
{
 ...
 */
void
PinholeCamera::spaceToPlane( const Eigen::Vector3d& P, Eigen::Vector2d& p, Eigen::Matrix< double, 2, 3 >& J ) const
{
    Eigen::Vector2d p_u, p_d;
    ...
    double dudx = fx*inv_denom;
    double dvdx = 0.0;
    double dudy = 0.0;
    double dvdy = fy*inv_denom;
    inv_denom   = -inv_denom * inv_denom;
    double dudz = fx*P( 0 ) * inv_denom;
    double dvdz = fy*P( 1 ) * inv_denom;
 ...
 */
void
PinholeCamera::distortion( const Eigen::Vector2d& p_u, Eigen::Vector2d& d_u, Eigen::Matrix2d& J ) const
{
    double k1 = mParameters.k1( );
    double k2 = mParameters.k2( );
    double p1 = mParameters.p1( );
...
cv::Mat
PinholeCamera::initUndistortRectifyMap(
