...
#define ROOT_POLYNOMIAL_ORDER ( 2 * FISHEYE_POLY_ORDER )
    ...
    SplineCamera( const std::string& cameraName,
                  int imageWidth,
                  int imageHeight,
                  double k2,
                  double k3,
...
template< typename T >
T
SplineCamera::r( T k2, T k3, T k4, T k5, T k6, T k7, T theta )
{
    // clang-format off
    return theta
        + k2 * theta * theta
...
template< typename T >
void
SplineCamera::spaceToPlane( const T* const params,
                            const T* const q,
                            const T* const t,
                            const Eigen::Matrix< T, 3, 1 >& P,
                            Eigen::Matrix< T, 2, 1 >& p )
